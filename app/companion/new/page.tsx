...
const NewCompanion = async () => {
  ...
  return (
    // FIX 1: Added 'justify-center' to vertically center content
    // Added 'py-12' for safe spacing on smaller screens
    <main className="flex flex-col items-center justify-center min-h-screen p-4 py-12">
      {canCreateCompanion ? (
        // Added 'max-w-3xl' to constrain width for better aesthetics
        <div className="w-full max-w-3xl flex flex-col gap-8">
            ...
            <article className="bg-white rounded-[2rem] p-6 md:p-10 border border-slate-100 shadow-sm">
                <CompanionForm />
            </article>
        </div>
      ) : (
        <article className="companion-limit bg-white p-10 rounded-[2rem] shadow-sm border border-slate-100 text-center max-w-lg">
